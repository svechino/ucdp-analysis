...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dcc.Dropdown(
                id='violence-filter',
                options=[{'label': v_label, 'value': v_key} for v_key, v_label in available_types_of_violence.items()],
                value='All',
                clearable=False
            )
        ], width=3)
    ...
    dbc.Row([
        dbc.Col(
            dcc.Graph(id='time-series-graph'), width=6
        ),
        dbc.Col(
            dcc.Graph(id='mortality-pie-chart'), width=6
        )
    ], className='mb-4'),
    dbc.Row([
        dbc.Col(
            dcc.Graph(id='event-map'), width=12
...
def update_graphs(selected_years, selected_decade, selected_region, selected_violence_type, is_dark):
    ...
    # Number of unique conflicts per year graph:
    conflicts_per_year = filtered_df.groupby('year')['conflict_name'].nunique().reset_index(name='unique_conflict_count')
    ...
    color_map = {
        'Side A Deaths': '#A9A9A9',  # light grey
        'Side B Deaths': '#2F4F4F',  # dark grey
        'Civilian Deaths': '#DC143C'  # Red
    }

    total_deaths['death_type'] = total_deaths['death_type'].map(legend_labels)
